...
public class test {
	public static void main(String[] args){
    	...
    	solve(coins.length, n, dp, new int[26], numPermutation, coins, chainPermutation, seen);
    	System.out.println(numPermutation[0]);
    }
	
	static void solve(int r, int c, int[][] dp, int[] trackedCoins, long[] numPermutation, int[] coinValues, long chainPermutation, HashMap<Integer, Integer> seen) {
		if(r < 0 || c < 0 || dp[r][c] == 0) 
			return;	
		if(r == 0 && c == 0) {
		...
		for(int i = c; i >= 0; i -= coinValues[r - 1]) {
			solve(r - 1, i, dp, trackedCoins, numPermutation, coinValues, chainPermutation, seen);	
			trackedCoins[coinValues[r - 1]]++;
			elementsSum++;
			chainPermutation = (chainPermutation * elementsSum) % 1000000007;   
			
			
			int key = trackedCoins[coinValues[r - 1]];
			if(!seen.containsKey(key)) {
				seen.put(key, (int) modInverse(trackedCoins[coinValues[r - 1]], 1000000007));
			}
			
			chainPermutation = (chainPermutation * seen.get(key)) % 1000000007;
		}
